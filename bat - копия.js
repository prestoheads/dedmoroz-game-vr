...
var _bats_source = [];
var _bats_source_rig = [];
var _bats_free = [];
var _bat_obj_tmp = null;
var _bat_rig_tmp = null;
var _new_bat_obj_tmp = null;
var _bats_i_tmp = null;
var _bats_i_tmp2 = null;
...
exports.init = function(elapsed_sensor) {
    for(_bats_i_tmp = 1; _bats_i_tmp <= m_config.BATS_TO_USE; _bats_i_tmp++) {
        _bats_free.push(true);

        _bat_obj_tmp = m_scs.get_object_by_name("Bat0" + _bats_i_tmp);
        m_train.setup_collector_score(_bat_obj_tmp);
        _bats_source.push(_bat_obj_tmp);

        _bat_rig_tmp = m_scs.get_object_by_name("BatArmature0" + _bats_i_tmp);
        m_anim.apply(_bat_rig_tmp, "BatFlapWingsAction");
        m_anim.set_behavior(_bat_rig_tmp, m_anim.AB_CYCLIC);
        _bats_source_rig.push(_bat_rig_tmp);
    }
...
function get_free_bat_index() {
    for(_bats_i_tmp = 0; _bats_i_tmp < _bats_free.length; _bats_i_tmp++) {
        if(_bats_free[_bats_i_tmp]) {
            break;
        }
    }
    if(_bats_i_tmp == _bats_free.length) {
        _bats_i_tmp = 0;
        console.log("OUT OF FREE BATS");
    }
    return _bats_i_tmp;
}

function use_bat(x, y, z) {
    _bats_i_tmp2 = get_free_bat_index();
    m_trans.set_translation(_bats_source[_bats_i_tmp2], x, y, z);

    m_anim.play(_bats_source_rig[_bats_i_tmp2]);

    _bats_free[_bats_i_tmp2] = false;
...
exports.setup_bats_generation = function (elapsed_sensor) {
    ...
    function generate_cb(obj, id) {
        ...
        if(dist_current < dist_last_generated - m_config.BAT_BETWEEN) {
            ...
            //_bats_genetated ++;
            setup_bat();
            use_bat(_bat_x, dist_current, _bat_z);
        }
